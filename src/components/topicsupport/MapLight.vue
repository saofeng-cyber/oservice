  ...
  <div class="chart-bg">
    ...
    <div class="view-type">
      <div
        ...
        :key="index"
        class="view-item"
        @click="handleChangeView(name, index)"
      >
        <img
        ...
        >
        <span>
          {{ name }} <span style="color:#8bc4d0;">364</span>
        </span>
      </div>
    </div>
    <div
...
export default {
  ...
  setup (props, context) {
    ...
    //let region = reactive({ id: 'yun_nan', name: '云南省' })
    ...
    const legendList = computed(() => {
      ...
      const list = [
        { label: `0 - ${Math.ceil(maxValue * 0.25)}`, min: 0, max: Math.ceil(maxValue * 0.25), color: '#F5FDFF' },
    ...
    const loadMapData = (val) => {
      const fileName = getfileName(val)
      if (val) {
        let nowjson = require('../../../public/config/govcharts/common/' + fileName)
        console.log(nowjson)
        //mapData
        setMapData(nowjson)
    ...
    const getOption = (chartsData) => {
      const option = {
        ...
        series: [
          { // 地图
            ...
            // data: chartsData ? chartsData : mapData
            data: mapData
          },
          // 柱状体的主干
          {
      ...
      console.log('mapData : ssss')
      console.log(mapData)
      return option
    }
    let tempList = ref('{0:123,1:234,2:234,3:153,4:243}')
    const setChartOption = () => {
      const option = getOption()
      charts.setOption(option, true)
    }
    const createMap = (d) => { // 查询地图数据，生成地图数据mapData
    ...
    const getfileName = (val) => { // 获取地图json文件路径
      console.log('vvvvvv')
      console.log(val)
      let file = '0'
    ...
    onMounted(() => {
      ...
      /*setTimeout(function () {
        const newRegion = { id: "yun_nan", name: "云南" };
        if (newRegion) {
          if (timer) {
            clearTimeout(timer)
            timer = null
