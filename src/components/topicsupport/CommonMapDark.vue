<template>
  <div class="chart-bg">
    <img
      class="map-bg"
      src="../../assets/map/Disk-A.png"
      alt=""
    >
    <img
      class="map-bg"
      src="../../assets/map/Disk-B.png"
      alt=""
    >
    <img
      class="map-bg"
      src="../../assets/map/Disk-C.png"
      alt=""
    >
    <el-button
      round
    ...
    <div class="view-type">
      ...
      >
        <img
          :src="require(`../../assets/map/${icons[index]}.png`)"
          alt=""
        >
        <span style="color:#ffffff">
          {{ item.name }} <span style="color:#66e2fb">{{ data.info[item.column] }}</span>
        </span>
      </div>
    </div>
    <div
...
export default {
  ...
  setup (props, context) {
    ...
    const getLegendList = () => {
      ...
      const list = [
        { label: `0 - ${Math.ceil(maxValue * 0.25)}`, min: 0, max: Math.ceil(maxValue * 0.25), color: '#003879' },
        { label: `${Math.ceil(maxValue * 0.25) + 1} - ${Math.ceil(maxValue * 0.5)}`, min: Math.ceil(maxValue * 0.25), max: Math.ceil(maxValue * 0.5), color: '#16365b' },
        { label: `${Math.ceil(maxValue * 0.5) + 1} - ${Math.ceil(maxValue * 0.75)}`, min: Math.ceil(maxValue * 0.5), max: Math.ceil(maxValue * 0.75), color: '#071f4a' },
        { label: `${Math.ceil(maxValue * 0.75) + 1} - ${maxValue}`, min: Math.ceil(maxValue * 0.75), max: maxValue, color: '#1e213f' },
      ]
    ...
    const scatterData = () => {
      return mapData.map((item) => {
        ...
        return [sysConfig.geoPostionCoordMap[item.name][0], sysConfig.geoPostionCoordMap[item.name][1] + item.value * lineMaxHeight(), item.value]
      })
    }
    // 柱状体的底部
    const scatterData2 = () => {
    ...
    const getOption = () => {
      ...
      const option = {
        tooltip: {
          show: true,
          backgroundColor: 'rgba(45, 48, 59, 0.56)',
    ...
    const createTooltip = (params) => {
      ...
      indexList.value.forEach((item, index) => {
        subItems.push(`<div style="margin: 0 5px; color: ${viewIndex.value === index ? '#fff' : ''}">
            ...
            <div style="display: block; white-space: nowrap;">${params.value2[index]}</div>
          </div>`)
      })
      let html = `
          <div style="position: absolute; top: 0; left: 0; bottom: 0; right: 0; filter: blur(20px);"></div>
          <div>
            <div style="font-size: 14px; font-weight: 500; color: #fff;" line-height: 20px;>${params.name}</div>
    ...
    const loadMapData = () => {
      ...
      $axios.get('/config/govcharts/map/config.json').then(d => {
        ...
        if (!props.data.area_name) {
          ...
          mapJsonPath = "/config/govcharts/map/citys/" + sysConfig.cityMap[areaName] + ".json"
        }
        if (loadedMap[areaName]) {
          ...
          $axios.get(mapJsonPath).then(mapJson => {
            ...
            mapFeatures.features.forEach(f => {
              if (sysConfig.geoPostionCoordMap[f.properties.name])// 使用自己的坐标
                geoCoordMap[f.properties.name] = sysConfig.geoPostionCoordMap[f.properties.name];
              else
                geoCoordMap[f.properties.name] = f.geometry.coordinates[0];
            })
    ...
    return {
      mapChart, loadMapData, legendList, icons, viewIndex, indexList, handleIndexChange
    }
  },
}
</script>
