  ...
  export default {
    ...
    props: {
      ...
      lineData: {
        ...
        default: () => ({
          source: 'yun_nan',
          dest: ['bei_jing', 'shan_dong', 'fu_jian', 'chong_qing', 'guang_dong']
        })
      },
    ...
    setup(props, context) {
      ...
      let icons = reactive(['Green-Bar', 'LightBlue-Bar', 'Yellow-Bar', 'DarkBlue-Bar', 'Purple-Bar', 'Plain-Bar']) // 严格按颜色顺序
      let viewIndex = ref(0) // 地图显示指标索引
      let isMapClick = ref(false)
      let isMapDbllick = ref(false)
      let timer = null
      let selectedProvince = null // 选中的省份
      ...
      const createMap = () => { // 查询地图数据，生成地图数据mapData
        ...
        if (selectedProvince) {
          allRegionNames = mapItems[region.id].map(item => ({ id: item.id, name: item.name, value: [...centerDatas[region.id][item.id]] }))
        }
        setChartOption()
      }
      ...
      const getMapLines = () => {
        const list = []
        for (let i = 0; i < props.lineData.dest.length; i++) {
          list.push({
            name: mapData.filter(item => item.id === props.lineData.dest[i])[0].name,
            coords: [
              [...centerDatas[region.id][props.lineData.source]],
              [...centerDatas[region.id][props.lineData.dest[i]]],
            ]
          })
