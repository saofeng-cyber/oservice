<template>
  <div>
    <div
      class="card-item"
      :class="{angle: !isActive}"
      :style="{background: isActive ? data.backgroundColor : ''}"
      @click="handleClick"
    >
      <div class="item-left">
        <div
          class="item-value"
          :style="{ color: isActive ? '' : data.color}"
        >
          {{ data.value }}
        </div>
        <div
          class="item-label"
          :style="{ color: isActive ? '' : '#1E222D'}"
        >
          {{ data.name }}
        </div>
      </div>
      ...
      >
        <img
          class="card-icon"
          :src="require(`../../assets/resource/${data.img + (isActive ? '-A' : '')}.png`)"
          alt=""
        >
      </div>
    </div>
