<template>
  <div class="page-wrapper">
      ...
      <el-col :span="14">
        <div class="card-left">
          ...
          <div class="button-img">
            <img src="@/assets/resource/toubu-light.png" alt="">
          </div>
          <div class="map-top">
            <el-row :gutter="12">
          ...
          <MapLines ref="mapChart" :data="mapData" :curCard="curCard" @handleMapSelect="handleMapSelect"
            @handleMapChange="handleMapChange"></MapLines>
        </div>
      </el-col>
      <el-col :span="10">
    ...
    <!-- 弹框组件 -->
    <div class="dialog">
      <el-dialog title="" v-model="dialogVisible" width="90%">
        <TableDialog></TableDialog>
      </el-dialog>
      <el-dialog title="" v-model="dialogVisibleTow" width="90%">
  ...
  export default {
    ...
    setup() {
      ...
      const handleMapChange = region => {
        ...
        mapHeader.value.setTitle('管理视图' + (region.id === '0' ? '' : '-' + region.name))
      ...
      const resChange = () => {
        const data = { jsonFile: 'speSupport-resVisual-resView-res-select.json', desc: '支撑专题-管理视图', spe: 'group' }
      ...
      const initData = () => {
        const data = { jsonFile: 'speSupport-resVisual-resView-all.json', desc: '支撑专题-管理视图', spe: 'group' }
        $axios.post(postUrl, data).then(d => {
      ...
      const initProvinceData = () => {
        const data = { jsonFile: 'speSupport-resVisual-resView-province.json', desc: '支撑专题-管理视图-省份视图', spe: 'group' }
        $axios.post(postUrl, data).then(d => {
          mapTopCardList.value = d.mapTopCardList
          mapData.value = d.mapData
      ...
      const initProvinceSelect = () => {
        const data = { jsonFile: 'speSupport-resVisual-resView-provselect.json', desc: '支撑专题-管理视图-省份视图', spe: 'group' }
        $axios.post(postUrl, data).then(d => {
          const val = d.customServiceData
          val.info.forEach(item => {
            item.url = getRingIconUrl({ color: item.color })
