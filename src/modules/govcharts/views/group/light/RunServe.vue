      ...
      <el-col :span="6">
        <div class="card-right card-1">
          <div class="top-title">
            <div class="float-title">
              <span class="i"></span>
              <span @click="gongdanXingqing('网络质量')" class="pointer"
                >网络质量</span
              >
            </div>
            <div class="float-title-right" @click="selfRefresh">
              <i class="el-icon-refresh" style="float: right; color: #000"></i>
            </div>
          </div>
          <div class="card-right-key">
            <el-select
              v-model="keyListValue"
              style="width: 360px"
              placeholder="网络关键指标监测"
              size="small"
            >
              <el-option
            ...
            >
              ...
              <el-radio-button
                label="省内"
                @click="
                  zhibiaoChanges('省内', 'EQUIPMENT_WARN');
                  initAlarmPie('省内');
                "
              >
                <div>省内</div>
        ...
        <div style="padding:0 15px">
            <div class="card-right-vip"></div>
        </div>
          <div
            style="width: 100%; height: calc(100% - 145px); text-align: center"
          >
            <WarnRanking2 ref="warnRank3"></WarnRanking2>
          </div>
        </div>

        <div class="card-right card-2">
          <div class="top-title" style="border-bottom: 1px solid #ccc">
            <div class="float-title">
              <span class="i"></span>
              <span @click="gongdanXingqing('服务质量')" class="pointer"
                >服务质量</span
              >
            </div>
            <div class="float-title-right" @click="selfRefresh">
...
import Map from "@/components/topicsupport/CommonMap2";
...
import RingPieChart from "@/components/common/RingPieChartFg";
import RingChart from "@/components/common/CircleProgressBarFg";
// import ExcelExport from "@/components/common/ExcelExport";
import WarnRanking2 from "@/components/emergency/WarnRanking2Light";
export default {
  components: {
    MapHeader,
    TableDialog,
  ...
  busiDataExport.push({
    ...
    let keyList = ref(["数据专线", "互联网", "语音", "APN", "MPLSVPN"]);
    let keyListValue = ref("")
    ...
    onMounted(() => {
      warnRank3.value.setRanking(rank3Data);
      mapHeader.value.setTitle("专线业务运营");
...
.card-right-vip{
    ...
    height: 8vh;
    width:100%;
    background: linear-gradient(to right,rgb(37, 165, 255),rgb(1, 104, 255));
    border-radius: 8px;
    z-index:99;
    box-shadow: 2.5px 2.5px 2.5px rgb(145, 145, 230);
}
.card-right.card-3 {
  height: 29vh;
  margin-top: 1vh;
