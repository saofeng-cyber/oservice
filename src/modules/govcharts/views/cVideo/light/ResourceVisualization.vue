  ...
  <div class="page-wrapper">
    ...
    >
      ...
      <el-col :span="10">
        ...
        <div>
          <BusSta ref="deviceBusSta"></BusSta>
        </div>
        <el-row
          v-if="false"
          :gutter="15"
...
import RingPieChart from '@/components/common/RingPieChart.vue'

import { provide } from "vue";

export default {
  components: {
    Map, MapTopCard, RingChart, MapHeader, WarnRanking, LineChart, SearchInfo, Warn, TableDialog, QueryCondition, BusSta,RingPieChart
  },
  setup () {
    ...
    const resChange = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-res-select.json', desc: '支撑专题-资源可视化', spe: 'cVideo' }
    ...
    const initData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-all.json', desc: '支撑专题-资源可视化', spe: 'cVideo' }
      $axios.post(postUrl, data).then(d => {
        ...
        deviceBusSta.value.setTitle('终端设备厂家占比');
        deviceBusSta.value.setValue([
          { name: '移动自研', num: 654 },
          { name: '华为', num: 754 },
          { name: '中兴', num: 554 },
          { name: '烽火', num: 454 },
        ...
        busSta.value.setTitle('支撑资源分析');
        busSta.value.setValue([
          { name: '企业宽带', num: 359 },
          { name: '互联网专线', num: 484 },
          { name: 'MPLS-VPN专线', num: 838 }
        ])
        ringPieChart.value.setTitle('终端设备型号占比');
        ringPieChart.value.setValues({
          title:"终端总数",
          info: [{num: '39.42%',name: "低成本C系列",color: "#0066ff",value: 993},
          {num: '39.01%',name: "桌面型D系列",color: "#2fc479",value: 540},
          {num: '26.56%',name: "行业定制H系列",color: "#fdb527",value: 529},
          {num: '26.56%',name: "专业型P系列",color: "#28b5dd",value: 445},
          {num: '26.56%', name: "保密会议终端P系列",color: "#d577ed", value: 415}]
        })
    ...
    const initProvinceData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-province.json', desc: '支撑专题-资源可视化-省份视图', spe: 'cVideo' }
      $axios.post(postUrl, data).then(d => {
        mapTopCardList.value = d.mapTopCardList
        mapData.value = d.mapData
    ...
    const initProvinceSelect = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-provselect.json', desc: '支撑专题-资源可视化-省份视图', spe: 'cVideo' }
      $axios.post(postUrl, data).then(d => {
        const val = d.customServiceData
        val.info.forEach(item => {
          item.url = getRingIconUrl({ color: item.color })
    ...
    return {
      ...
      handleResChange, showLeftMenu, leftMenus, openLeftMenu, searchShow, getCondition, getTableMessage, dialogVisible, dialogVisibleTow, deviceBusSta, busSta,ringPieChart
    }
  }
}
</script>
