  ...
  <div class="page-wrapper">
    ...
    >
      ...
      <el-col :span="10">
        ...
        <div style="margin-bottom: 15px;background-color:#fff;height: 230px;">
...
export default {
  components: {
    Map, MapTopCard, RingChart, MapHeader, WarnRanking, LineChart, SearchInfo, Warn, TableDialog, QueryCondition, BusSta, BarChart,ListTable
  },
  setup () {
    ...
    const resChange = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-res-select.json', desc: '支撑专题-资源可视化', spe: 'sCamera' }
    ...
    const initData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-all.json', desc: '支撑专题-资源可视化', spe: 'sCamera' }
      $axios.post(postUrl, data).then(d => {
        ...
        refBarChart.value.setTitle('千里眼终端增长排名数')
        ...
        listTable.value.setTitle('终端型号排名')
        listTable.value.setValues({
          ranking: '排名',
          terminalType: "终端型号",
          terminalNum: "终端数量",
          factory: "厂家",
          tetvalue: ["S21", "S22", "M21", "A21"],
          tenvalue: ["241", "24", "21", "4"],
          favalue: ["海康威视", "大华", "大华", "海康威视"]
    ...
    const initProvinceData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-province.json', desc: '支撑专题-资源可视化-省份视图', spe: 'sCamera' }
      $axios.post(postUrl, data).then(d => {
        mapTopCardList.value = d.mapTopCardList
        mapData.value = d.mapData
    ...
    const initProvinceSelect = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-provselect.json', desc: '支撑专题-资源可视化-省份视图', spe: 'sCamera' }
      $axios.post(postUrl, data).then(d => {
        const val = d.customServiceData
        val.info.forEach(item => {
          item.url = getRingIconUrl({ color: item.color })
    ...
    return {
      ...
      handleResChange, showLeftMenu, leftMenus, openLeftMenu, searchShow, getCondition, getTableMessage, dialogVisible, dialogVisibleTow, busSta, refBarChart,listTable
    }
  }
}
</script>
