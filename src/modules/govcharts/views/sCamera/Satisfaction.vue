<template>
  <div class="page-wrapper">
    <el-row class="row-1" :gutter="15">
          ...
          <div class="map-top">
            <el-row :gutter="12">
              <el-col :span="4" v-for="(item, index) in mapTopCardList" :key="index">
                <MapTopCard :data="item" :hideImg="true" :isActive="curCard === item.name"
      ...
      <el-col :span="10">
        <div class="card-right card-with-title card-1">
          <img :src="require(`../../../../assets/temporaryBG/group_right1.png`)" alt="" />
        </div>
        <div class="card-right card-with-title card-2">
          <RanList :tableData="tableData" tableHeigh="180"></RanList>
        </div>
        <div class="card-right card-with-title card-3">
          <ContrastChart ref="ContrastChart" :title="'开通满意度一各省季度对比分析'"></ContrastChart>
        </div>
      </el-col>
    </el-row>
    <el-row class="row-2" :gutter="15">
      ...
      <el-col :span="12">
        <div class="card-bottom card-with-title card-2">
          <LineChart ref="refLineChart" :title="'开通满意度近30日增长趋势'" @lineChartRefresh="lineChart_Refresh" />
        </div>
      </el-col>
    </el-row>
  </div>
