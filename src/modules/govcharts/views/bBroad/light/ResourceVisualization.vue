  ...
  <div class="page-wrapper">
    ...
    >
      ...
      <el-col :span="10">
        ...
        <div style="margin-bottom: 15px;">
          <TradeDis ref="tradeDis"></TradeDis>
        </div>
        <div style="background-color:#fff;">
          <NetResource ref="honorNetRes5G" />
        </div>
        <el-row
          v-if="false"
          :gutter="15"
...
import TradeDis from "@/components/common/TradeDis";
import NetResource from '@/components/topicsupport/NetResourceLight.vue'
import { provide } from "vue";

export default {
  components: {
    Map, MapTopCard, RingChart, MapHeader, WarnRanking, LineChart, SearchInfo, Warn, TableDialog, QueryCondition, BusSta, TradeDis, NetResource
  },
  setup () {
    ...
    const resChange = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-res-select.json', desc: '支撑专题-资源可视化', spe: 'bBroad' }
    ...
    const initData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-all.json', desc: '支撑专题-资源可视化', spe: 'bBroad' }
      $axios.post(postUrl, data).then(d => {
        ...
        busSta.value.setTitle('接入类型分析');
        busSta.value.setValue([
          { name: 'FTTH', num: '60.5%' },
          { name: 'FTTB', num: '39.5%' }
        ])

        tradeDis.value.setTitle('企业网关厂家排名');
        tradeDis.value.setValue([
          { name: '中小企业', num: 993, rate: 29.42 },
          { name: '邮局', num: 540, rate: 19.01 },
          { name: '党政', num: 529, rate: 16.56 },
          { name: '金融', num: 445, rate: 15.56 },
          { name: '教育', num: 415, rate: 11.56 },
        ])
        honorNetRes5G.value.setTitle('支撑资源分析');
        honorNetRes5G.value.setValues([
          { "name": "BRAS", "value": 112, "img": "UPF.png" },
          { "name": "OLT", "value": 355, "img": "SMF.png" },
          { "name": "ONU", "value": 571, "img": "AMF.png" },
        ])
      }).then(() => {
    ...
    const initProvinceData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-province.json', desc: '支撑专题-资源可视化-省份视图', spe: 'bBroad' }
      $axios.post(postUrl, data).then(d => {
        mapTopCardList.value = d.mapTopCardList
        mapData.value = d.mapData
    ...
    const initProvinceSelect = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-provselect.json', desc: '支撑专题-资源可视化-省份视图', spe: 'bBroad' }
      $axios.post(postUrl, data).then(d => {
        const val = d.customServiceData
        val.info.forEach(item => {
          item.url = getRingIconUrl({ color: item.color })
    ...
    return {
      ...
      handleResChange, showLeftMenu, leftMenus, openLeftMenu, searchShow, getCondition, getTableMessage, dialogVisible, dialogVisibleTow, busSta, tradeDis, honorNetRes5G
    }
  }
}
</script>
