  ...
  <div class="page-wrapper">
    ...
    >
      <el-col :span="14">
        <div class="card-left">
          ...
          <div class="button-img">
            <img
              src="@/assets/resource/cNetHeader-img.png"
              alt=""
            >
          </div>
          <div class="map-top">
      ...
      <el-col :span="10">
        ...
        <div style="margin-bottom: 15px;background-color:#fff;padding:15px 10px;">
          <div class="top-title">
            <div class="float-title">
              <span class="i"></span>云互联业务统计
...
export default {
  components: {
    Map, MapTopCard, RingChart, MapHeader, WarnRanking, LineChart, SearchInfo, Warn, TableDialog, QueryCondition, Rotate, TradeDis, NumberWrap
  },
  setup () {
    ...
    const resChange = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-res-select.json', desc: '支撑专题-资源可视化', spe: 'cNet' }
    ...
    const initData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-all.json', desc: '支撑专题-资源可视化', spe: 'cNet' }
      $axios.post(postUrl, data).then(d => {
        ...
        tradeDis.value.setTitle('集团客户行业TOP5');
    ...
    const initProvinceData = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-province.json', desc: '支撑专题-资源可视化-省份视图', spe: 'cNet' }
      $axios.post(postUrl, data).then(d => {
        mapTopCardList.value = d.mapTopCardList
        mapData.value = d.mapData
    ...
    const initProvinceSelect = () => {
      const data = { jsonFile: 'speSupport-resVisual-resView-provselect.json', desc: '支撑专题-资源可视化-省份视图', spe: 'cNet' }
      $axios.post(postUrl, data).then(d => {
        const val = d.customServiceData
        val.info.forEach(item => {
          item.url = getRingIconUrl({ color: item.color })
    ...
    return {
      ...
      handleResChange, showLeftMenu, leftMenus, openLeftMenu, searchShow, getCondition, getTableMessage, dialogVisible, dialogVisibleTow, tradeDis
    }
  }
}
</script>
