    ...
    <el-row class="row-1" :gutter="15">
          ...
          <div class="button-img">
            <el-row :gutter="20">
              ...
              <el-col :span="1" style="padding-top: 6px">
                <i class="el-icon-search btnStyle" @click="searchShow"></i>
              </el-col>
            </el-row>
      ...
      <el-col :span="10" style="height: 100%">
        ...
        <div class="card-right card-4">
          <div class="card-right card-4-left" style="width: 49%">
            <ButList ref="butList"></ButList>
          </div>
          <div class="card-right card-4-right" style="width: 49%">
...
import Map from "@/components/topicsupport/CommonMapDark";
import ButList from "@/components/common/ButList";
import RotateData from "@/components/common/RotateDataDark";
import BarChart from "@/components/common/BarChartAlarmDark";
    ...
    const queryData = function (orgType, city) {
      ...
      $axios
        .post(u, {
          ...
          industryName:
            allindustries.value == "全部行业" ? "" : allindustries.value || "",
          selectcustomer:
            customer.value == "选择客户" ? "" : customer.value || "",
        ...
        .then((res) => {
          ...
          equipmentStatus.value.setValue([
            {
              src: require("@/assets/resource/AMF.png"),
              name: "AMF",
              num: res.data.EQUIPMENT_AMF || 0,
              color: "#ffa800",
            },
            {
              src: require("@/assets/resource/SMF.png"),
              name: "SMF",
              num: res.data.EQUIPMENT_SMF || 0,
              color: "#0eacda",
            },
            {
              src: require("@/assets/resource/UPF.png"),
              name: "UPF",
              num: res.data.EQUIPMENT_UPF || 0,
              color: "#0066ff",
            },
            {
              src: require("@/assets/resource/AMF.png"),
              name: "无线基站",
              num: res.data.EQUIPMENT_WIRELESS || 0,
              color: "#2ec478",
            },
            {
              src: require("@/assets/resource/AMF.png"),
              name: "MEC",
              num: res.data.EQUIPMENT_MEC || 0,
              color: "#4b5575",
            },
            {
              src: require("@/assets/resource/AMF.png"),
              name: "IP承载网",
              num: res.data.EQUIPMENT_IP || 0,
              color: "#f81d22",
            },
          ]);
          ...
          butList.value.setValues({
            but: [
              { name: "核心网告警", num: res.data.ALARM_CORE || 0 },
            ...
            device: [
              "设备告警数",
              res.data.AMF_EQUIPMENT || 0,
              res.data.SMF_EQUIPMENT || 0,
              res.data.UPF_EQUIPMENT || 0,
            ],
            alarmdevices: [
              "告警设备数",
              res.data.AMF_ALARM || 0,
              res.data.SMF_ALARM || 0,
              res.data.UPF_ALARM || 0,
            ],
            dnn: [
              "影响DNN数",
              res.data.AMF_DNN || 0,
          ...
          if (null != res.data.AFFECT_CUSTOMER) {
            if (parseInt(res.data.AFFECT_CUSTOMER) < 10) {
              ...
              parseInt(res.data.AFFECT_CUSTOMER) >= 10 &&
              parseInt(res.data.AFFECT_CUSTOMER) < 100
            ) {
              number = [
                "0",
                res.data.AFFECT_CUSTOMER.substring(0, 1),
                res.data.AFFECT_CUSTOMER.substring(1, 2),
              ];
            } else if (parseInt(res.data.AFFECT_CUSTOMER) > 100) {
              number = [
                res.data.AFFECT_CUSTOMER.substring(0, 1),
                res.data.AFFECT_CUSTOMER.substring(1, 2),
                res.data.AFFECT_CUSTOMER.substring(2, 3),
              ];
            }
          }
          butProgress.value.setValues({
            but: [
              { name: "设备告警", num: parseInt(res.data.TOP_CARD_BC) },
